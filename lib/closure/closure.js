...
function ff(x){var tmp=3;function f(y){console.log(x+y+ ++tmp)}f(10)}ff(2);//16
ff(2);//16
// 这样不管执行多少次 都会console.log()出 16 ，因为bar 能访问 的参数tmp 也都能伪证问 x 
// 但是这还不是闭包，当我把function 里的function return 回来的时候，才是一个闭包，
function ffreturn(x){var tmp=3;return function(y){console.log(x+y+ ++tmp)}}var ffreturn1=ffreturn(2);ffreturn1(10);//16
ffreturn1(10);//17
...
function fobject(x){var tmp=3;return function(y){console.log(x+y+tmp);x.memb=x.memb?x.memb+1:1;console.log(x.memb)}}var age=new Number(2);var bar=fobject(age);bar(10);